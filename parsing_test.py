...
# basically, do dfs search and process all of the sceneblocks [and eventually
# itemblocks]. Also find the startstates and use them to construct main.
class Constructor:    
    def __init__(self):
    ...
    def scene_gen(self, scene):
        ...
        for c in scene.children:
            if c.type == "sceneid":
                ...
                #there's a good chance this is going to have the wrong indentation when there are multiple commands
    ...
    def process_statements(self, statements):
        # this is where statements are going to get processed. right now, only need to figure out "say" and "win"
        commands = []
        if len(statements) > 0:
            if statements[0].children[0].type == "statementlist":
                for s in statements[0].children[0].children:
                    if s.children[0].type == "say":
                        commands.append("print \"" + s.children[0].children[0].value + "\"")
